...
let workerQueue = dispatch_queue_create("com.darrenmartinleith.worker", attr)
...
dispatch_async(workerQueue) { 
	print("ASYNCHRONOUS: \(getCurrentQueueName())")
}
...
dispatch_sync(workerQueue) {
	print("SYNCHRONOUS \(getCurrentQueueName())")
}
...
let concurrentQueue = dispatch_queue_create("com.darrenmartinleith.com.concurrentQueue", DISPATCH_QUEUE_CONCURRENT)
